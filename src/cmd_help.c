...
void Cmd_help(int n, char* tr[]) {
	...
	help("showenv", "[-environ|-addr]", "Shows the process environment\n\t-environ: access through external variable environ\n\t-addr: shows addresses of envp and environ");
	help("jobs", "", "Lists background processes launched from the shell\n\tShows PID, start time, priority, status and command");
	help("deljobs", "[-term|-sig]", "Deletes processes from the background list\n\t-term: deletes processes that terminated normally\n\t-sig: deletes processes terminated by a signal");
